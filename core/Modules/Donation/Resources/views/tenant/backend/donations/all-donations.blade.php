                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <x-admin.header-wrapper>
                            <x-slot name="left">
                                <h4 class="card-title mb-5">{{__('All Donation')}}</h4>
                                <x-bulk-action permissions="donation-delete"/>
                            </x-slot>
                            <x-slot name="right" class="d-flex">
                                <form action="{{route('tenant.admin.donation')}}" method="get">
    ...
    <script>
        (function ($) {
            ...
            $(document).ready(function () {
                <x-bulk-action-js :url="route('tenant.admin.donation.bulk.action')"/>
            })
        })(jQuery)
    </script>
    <x-media-upload.js/>
        ...
        $(function () {
            $(document).ready(function () {
                $('.table-wrap > table').DataTable({
                    processing: true,
                    serverSide: true,
                    ajax: "{{ route('tenant.admin.donation',['lang' => $default_lang]) }}",
                    columns: [
