    ...
    <nav class="navbar navbar-area  navbar-expand-lg ">
        <div class="container container-two nav-container">
            ...
            <div class="nav-right-content">
                @php
                    $event_page_route = route('tenant.dynamic.page',get_dynamic_page_name_by_id(get_static_option('event_page')));
                    $user_dynamic_route = get_static_option('event_top_event_button_'.get_user_lang().'_url');
                    $condition = !empty($user_dynamic_route) ? $user_dynamic_route : $event_page_route;
                @endphp
                <div class="btn-wrapper">
                    <a href="{{ $condition }}" class="cmn-btn">{{ get_static_option('event_top_event_button_'.get_user_lang().'_text') ?? __('Events') }}</a>
                </div>
            </div>
        </div>
    </nav>
