                    ...
                    <div class="card-body">
                        <x-admin.header-wrapper>
                            ...
                            <x-slot name="right" class="d-flex">
                                ...
                                <x-link-with-popover  url="{{route('landlord.admin.seeder.portfolio.index')}}" extraclass="ml-3">
                                    {{__('Go Back')}}
                                </x-link-with-popover>
                            ...
                            <table class="table table-default table-striped table-bordered">
                                ...
                                @foreach($all_data_decoded->data ?? [] as $data)
                                    @php
                                        ...
                                        $client_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->client)) ?? [];
                                        $design_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->design)) ?? [];
                                        $typography_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->typography)) ?? [];
                                    @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#"
                                               ...
                                               data-client="{{$client_decoded[$lang_slug] ?? ''}}"
                                               data-design="{{$design_decoded[$lang_slug] ?? ''}}"
                                               data-typography="{{$typography_decoded[$lang_slug] ?? ''}}"
            ...
            <div class="modal-content ">
                ...
                <form action="" method="post" enctype="multipart/form-data" class="donation_category_seeder_edit_form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="order_status">{{__('Client')}}</label>
                            <input type="text" name="client" class="form-control client">
                        </div>

                        <div class="form-group">
                            <label for="order_status">{{__('Design')}}</label>
                            <input type="text" name="design" class="form-control design">
                        </div>

                        <div class="form-group">
                            <label for="order_status">{{__('Typography')}}</label>
                            <input type="text" name="typography" class="form-control typography">
                        </div>
    ...
    <script>
        $(document).ready(function($){
            ...
            $(document).on('click','.donation_category_seeder_edit_btn',function(){
                ...
                let client = el.data('client');
                let design = el.data('design');
                let typography = el.data('typography');
                let description = el.data('description');

                form.find('.donation_id').val(id);
                form.find('.title').val(title);
                form.find('.client').val(client);
                form.find('.design').val(design);
                form.find('.typography').val(typography);
