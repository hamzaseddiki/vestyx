                ...
                <div class="card">
                    <div class="card-body">
                        <x-admin.header-wrapper>
                            <x-slot name="left">
                                <h4 class="header-title">{{__('Price Plan Demo Data')}}</h4>
                            </x-slot>
                            <x-slot name="right" class="d-flex">
                                ...
                                <x-link-with-popover  url="{{route('landlord.admin.seeder.price.plan.index')}}" extraclass="ml-3">
                                    {{__('Go Back')}}
                                </x-link-with-popover>
                            ...
                            <table class="table table-default table-striped table-bordered">
                                ...
                                @foreach($all_data_decoded->data ?? [] as $data)
                                    @php
                                        ...
                                        $features_decoded = (array) json_decode($data->features) ?? [];
                                        $not_features_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->not_available_features)) ?? [];
                                    @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#"
                                               ...
                                               data-features="{{$features_decoded[$lang_slug] ?? ''}}"
                                               data-not_features="{{$not_features_decoded[$lang_slug] ?? ''}}"
            ...
            <div class="modal-content ">
                ...
                <form action="" method="post" enctype="multipart/form-data" class="donation_category_seeder_edit_form">
                    ...
                    <div class="modal-body">
                        ...
                        <x-summernote.textarea label="{{__('Available Features')}}" name="features" class="features" info="{{__('Separate feature by entering (comma) end of the line')}}"/>
                        <x-summernote.textarea label="{{__('Not Available Features')}}" name="not_available_features" class="not_available_features" info="{{__('Separate not available feature by entering (comma) end of the line')}}"/>
    ...
    <script>
        $(document).ready(function($){
            ...
            $(document).on('click','.donation_category_seeder_edit_btn',function(){
                ...
                let features = el.data('features');
                let not_features = el.data('not_features');

                form.find('.donation_id').val(id);
                form.find('.title').val(title);
                $('.features').summernote('code',features);
                $('.not_available_features').summernote('code',not_features);
