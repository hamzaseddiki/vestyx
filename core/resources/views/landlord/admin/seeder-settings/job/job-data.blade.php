                ...
                <div class="card">
                    <div class="card-body">
                        <x-admin.header-wrapper>
                            <x-slot name="left">
                                <h4 class="header-title">{{__('Job Demo Data')}}</h4>
                            </x-slot>
                            <x-slot name="right" class="d-flex">
                                ...
                                <x-link-with-popover  url="{{route('landlord.admin.seeder.job.index')}}" extraclass="ml-3">
                                    {{__('Go Back')}}
                                </x-link-with-popover>
                            ...
                            <table class="table table-default table-striped table-bordered">
                                ...
                                @foreach($all_data_decoded->data ?? [] as $data)
                                    @php
                                        ...
                                        $designation_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->designation)) ?? [];
                                        $job_location_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->job_location)) ?? [];
                                        $company_name_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->company_name)) ?? [];
                                    @endphp
                                    <tr>
                                        ...
                                        <td>
                                            <a href="#"
                                               ...
                                               data-job_location="{{$job_location_decoded[$lang_slug] ?? ''}}"
                                               data-company_name="{{$company_name_decoded[$lang_slug] ?? ''}}"
            ...
            <div class="modal-content ">
                ...
                <form action="" method="post" enctype="multipart/form-data" class="donation_category_seeder_edit_form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="order_status">{{__('Job Location')}}</label>
                            <input type="text" name="job_location" class="form-control job_location">
                        </div>

                        <div class="form-group">
                            <label for="order_status">{{__('Company Name')}}</label>
    ...
    <script>
        $(document).ready(function($){
            ...
            $(document).on('click','.donation_category_seeder_edit_btn',function(){
                ...
                let job_location = el.data('job_location');
                let company_name = el.data('company_name');
                let description = el.data('description');

                form.find('.donation_id').val(id);
                form.find('.title').val(title);
                form.find('.designation').val(designation);
                form.find('.job_location').val(job_location);
                form.find('.company_name').val(company_name);
