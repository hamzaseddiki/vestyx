                ...
                <div class="card">
                    <div class="card-body">
                        <x-admin.header-wrapper>
                            <x-slot name="left">
                                <h4 class="header-title">{{__('Gallery Demo Data')}}</h4>
                            </x-slot>
                            <x-slot name="right" class="d-flex">
                                ...
                                <x-link-with-popover  url="{{route('landlord.admin.seeder.gallery.index')}}" extraclass="ml-3">
                                    {{__('Go Back')}}
                                </x-link-with-popover>
                            ...
                            <table class="table table-default table-striped table-bordered">
                                ...
                                @foreach($all_data_decoded->data ?? [] as $data)
                                    @php
                                        ...
                                        $subtitle_decoded = (array) json_decode(get_data_without_extra_space_or_new_line($data->subtitle)) ?? [];
                                    @endphp
                                    <tr>

                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $title_decoded[$lang_slug] ?? '' }}</td>
                                        <td>{{ \Illuminate\Support\Str::words($subtitle_decoded[$lang_slug] ?? '',20) }}</td>
                                        <td>
                                            <a href="#"
                                               data-id="{{$data->id}}"
                                               data-title="{{$title_decoded[$lang_slug] ?? ''}}"
                                               data-subtitle="{{$subtitle_decoded[$lang_slug] ?? ''}}"
            ...
            <div class="modal-content ">
                ...
                <form action="" method="post" enctype="multipart/form-data" class="donation_category_seeder_edit_form">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="order_status">{{__('Subtitle')}}</label>
