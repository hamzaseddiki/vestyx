                ...
                <div class="card">
                    <div class="card-body table-responsive">
                       ...
                       <div class="alert alert-warning my-4">
                           <small class="text-primary">{{__('Add English (USA) language, to translate all tenant language,
                            becuase by default tenant default language is English(USA). If you dont want to show English (USA) in your frontend, then you can set this as draft.')}}</small>
                       </div>
                       @endif
                        <table class="table table-default">
                            ...
                            @foreach($all_lang as $data)
                                <tr>
                                    ...
                                    <td>
                                        @if($data->status == 1)
                                        <span class="badge badge-success">{{ \App\Enums\StatusEnums::getText($data->status) }}</span>
                                        @else
                                          <span class="badge badge-danger">{{ \App\Enums\StatusEnums::getText($data->status) }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        @can('language-edit')
                        ...
                        <form action="{{route(route_prefix().'admin.languages.new')}}" method="post" enctype="multipart/form-data" class="new_language_form">
                         ...
                         @if(!tenant())
                          ...
                          @endif
                            ...
                            @if(tenant())
                                @php
                                    $all_central_langs = (array) json_decode(get_static_option_central('all_central_langs')) ?? [];
                               @endphp
                            <div class="form-group">
                                <label for="name">{{__('Language')}}</label>
                                <input type="hidden" name="name">
                                    <select name="language_select" class="form-control">
                                        @foreach($all_central_langs as $key=> $item)
                                            <option value="{{$key}}" lang="{{$key}}">{{$item}}</option>
                                         @endforeach
                                    </select>
                                <small class="text-primary">{{__('You have only available these languages, if you need to add more language then contact to the administrator..!')}}</small>
                            </div>
                            @endif
            ...
            <div class="modal-content">
                ...
                <form action="{{route(route_prefix().'admin.languages.update')}}" class="edit_language_form" method="post">
                    <div class="modal-body">
                        ...
                        @if(tenant())
                            @php
                                $all_central_langs = json_decode(get_static_option_central('all_central_langs'));
                            @endphp
                            <div class="form-group">
                                <label for="name">{{__('Language')}}</label>
                                <input type="hidden" name="name">
                                <select name="language_select" class="form-control">
                                    @foreach($all_central_langs ?? [] as $item)
                                            <option value="{{$key}}" lang="{{$key}}">{{$item}}</option>
                                    @endforeach
                                </select>
                            </div>
