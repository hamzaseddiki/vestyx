                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route(route_prefix() . 'admin.payment.gateway.update') }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            <input name="gateway" type="hidden" value="{{ $gateway->name ?? '' }}">
                                    ...
                                    <div class="card">
                                        ...
                                        <div class="card-body">
                                            ...
                                            @if (@$gateway->name === 'paystack')
                                                ...
                                                <input class="form-control mb-2" type="text" value="{{ route('landlord.frontend.paystack.ipn') }}" readonly />
                                            @endif

                                            <div class="form-group">
                                                <label for="instamojo_gateway"><strong>{{ __('Enable/Disable') }} {{ str_replace('_', ' ', ucfirst(@$gateway->name)) }}</strong></label>
                                                <input name="{{ @$gateway->name }}_gateway" type="hidden">
                                                <label class="switch">
                                                    <input name="{{ @$gateway->name }}_gateway" type="checkbox" @if (@$gateway->status === 1) ) checked @endif>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <label class="switch">
                                                    <input name="{{ @$gateway->name }}_test_mode" type="checkbox" @if (@$gateway->test_mode === 1) checked @endif>
                                                    <span class="slider onff"></span>
                                                </label>
                                            </div>

                                            <x-landlord-others.edit-media-upload-image name="{{ @$gateway->name . '_logo' }}" label="{{ __('Logo') }} {{ str_replace('_', ' ', ucfirst(@$gateway->name)) }}" :value="@$gateway->image" size="100*100" />
                                            ...
                                            @foreach ($credentials as $cre_name => $cre_value)
                                                @if (!in_array(@$gateway->name, ['manual_payment_', 'bank_transfer', 'kinetic']))
                                                    <div class="form-group">
                                                        ...
                                                        <input class="form-control" name="{{ @$gateway->name . '_' . $cre_name }}" type="text" value="{{ $cre_value }}">
                                                ...
                                                @if (in_array(@$gateway->name, ['manual_payment_', 'bank_transfer']))
                                                    ...
                                                    @elseif($loop->index == 1)
                                                        <div class="form-group">
                                                            <label>{{ str_replace('_', ' ', ucfirst($cre_name)) }}</label>
                                                            <textarea class="form-control" name="{{ @$gateway->name . '_' . $cre_name }}" rows="4">{!! $cre_value !!}</textarea>
                                                        </div>
                            ...
                            <button class="btn btn-primary mt-4 pl-4 pr-4" type="submit">{{ __('Update Changes') }}</button>
                        </form>
                    </div>
                </div>
            </div>
