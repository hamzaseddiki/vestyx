@if(get_static_option('tenant_default_theme') == 'hotel-booking')
    <div class="col-4">
        <h4 class="mb-4 text-primary">{{__('Theme (Hotel Booking)')}}</h4>
        ...
        <h4 class="header-title margin-top-80 mt-3">{{__("Heading Font (Hotel Booking)")}}</h4>
        <div class="form-group">
            <label for="heading_font">{{__('Heading Font')}}</label>
            <label class="switch">
                <input type="checkbox" name="heading_font_{{$suffix}}" class="heading_font"
                       @if(!empty(get_static_option('heading_font_'.$suffix))) checked @endif id="heading_font"
                       data-theme="{{$suffix}}">
        ...
        <div class="heading_container">
            <div class="form-group">
                ...
                <select class="form-control nice-select wide heading_font_family" name="heading_font_family_{{$suffix}}"
                        id="heading_font_family" data-theme="{{$suffix}}">
                    @foreach($google_fonts as $font_family => $font_variant)
                        <option value="{{$font_family}}"
            ...
            <div class="form-group margin-top-50">
                ...
                @php
                    ...
                    $get_font_family_variants = property_exists($google_fonts,$font_family_selected) ? (array) $google_fonts->$font_family_selected : ['variants' => array('regular')];
                @endphp
                <select class="form-control nice-select wide heading_font_variant_{{$suffix}}" multiple
                        name="heading_font_variant_{{$suffix}}[]" id="heading_font_variant">
                    @foreach($get_font_family_variants['variants'] as $variant)
                        @php
