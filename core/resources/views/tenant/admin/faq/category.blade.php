...
@section('title') {{__('All Faq Categories')}} @endsection
        ...
        <div class="card">
            <div class="card-body">
                ...
                <x-admin.header-wrapper>
                    ...
                    <x-slot name="left">
                        <h4 class="card-title mb-5">{{__('All Faq Categories')}}</h4>
                            ...
                            <tr>
                                ...
                                @can('testimonial-edit')
                                    <a href="#"
                                       ...
                                       title="{{__('Edit')}}"
                                       data-id="{{$data->id}}"
                                       data-action="{{route('tenant.admin.faq.category.update')}}"
                                       data-title="{{$data->getTranslation('title',$default_lang)}}"
                                       data-status="{{$data->status}}">
                                        <i class="las la-edit"></i>
                                    </a>
                                    @endcan
                                    <x-delete-popover url="{{route('tenant.admin.faq.category.delete', $data->id)}}"/>
                                </td>
                            </tr>
                        @endforeach
                    </x-slot>
        ...
        <div class="modal fade" id="new_testimonial" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form action="{{route('tenant.admin.faq.category')}}" method="post" enctype="multipart/form-data">
    ...
    @can('testimonial-edit')
        <div class="modal fade" id="testimonial_item_edit_modal" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="staticBackdropLabel">{{__('Edit Faq Category Item')}}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
...
@section('scripts')
    ...
    <script>
        $(document).ready(function($){
            ...
            <x-bulk-action-js :url="route('tenant.admin.faq.category.bulk.action')" />
