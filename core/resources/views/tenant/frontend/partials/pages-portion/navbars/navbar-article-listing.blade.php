    ...
    <nav class="navbar navbar-area aticle-nav navbar-expand-lg ">
        <div class="container container-two nav-container">
            ...
            <div class="nav-right-content">
                @php
                    $article_page_route = route('tenant.dynamic.page',get_dynamic_page_name_by_id(get_static_option('knowledgebase_page')));
                    $user_dynamic_route = get_static_option('article_top_job_button_'.get_user_lang().'_url');
                    $condition = !empty($user_dynamic_route) ? $user_dynamic_route : $article_page_route;
                @endphp
                <div class="btn-wrapper">
                    <a href="{{$condition}}" class="cmn-btn1">{{get_static_option('article_top_job_button_'.get_user_lang().'_text') ?? __('Articles') }}</a>
                </div>
            </div>
        </div>
    </nav>
