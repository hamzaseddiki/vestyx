    ...
    <nav class="navbar navbar-area  navbar-expand-lg ">
        <div class="container container-two nav-container">
            ...
            <div class="nav-right-content">
                @php
                    $ticket_page_route = route('tenant.frontend.support.ticket');
                    $user_dynamic_route = get_static_option('ticket_top_job_button_'.get_user_lang().'_url');
                    $condition = !empty($user_dynamic_route) ? $user_dynamic_route : $ticket_page_route;
                @endphp
                <div class="btn-wrapper">
                    <a href="{{$condition}}" class="cmn-btn1">{{get_static_option('ticket_top_job_button_'.get_user_lang().'_text') ?? __('Create Ticket') }}</a>
                </div>
            </div>
        </div>
    </nav>
