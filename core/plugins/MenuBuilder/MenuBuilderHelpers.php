...
class MenuBuilderHelpers
{
    ...
    public function get_post_type_page_list($lang): string
    {
        ...
        $all_supported_menu = $this->menu_builder_setup_instance->register_dynamic_menus();

        $all_supported_menu_filtered = [];
        
        foreach ($all_supported_menu as $key => $mItem){
            if($mItem['support'] === 1 && is_null(tenant())){
                $all_supported_menu_filtered[$key] = $mItem;
                
            }elseif($mItem['support'] === 2 && !is_null(tenant())){
                $all_supported_menu_filtered[$key] = $mItem;
                
            }elseif($mItem['support'] === 0){
                $all_supported_menu_filtered[$key] = $mItem;
            }
        }
        
        return $this->render_dynamic_pages_list($all_supported_menu_filtered,$lang);
            ...
            foreach ($all_items as $item){
                ...
                if ($page_details['query'] === 'old_lang'){
                    ...
                    $title = $item->lang_query->$title_param ?? '';
                      ...
                      $title = $item->getTranslation('title',get_user_lang()) ?? '';
                }
