...
class TenantPopularAppointmentWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Appointment::where(['status' => 1])->take($blog_items)->orderBy('views','desc')->get();

        $blogs_markup = '';
        foreach ($blog_posts as $post){

            $image = render_background_image_markup_by_attachment_id($post->image,'','thumb');
            $route = route(route_prefix().'frontend.appointment.single',$post->slug);
            $title = Str::words($post->title,10);
...
HTML;
    ...
    {
        return __('Popular Appointments');
    }
}
