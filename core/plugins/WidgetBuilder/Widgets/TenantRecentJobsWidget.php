...
class TenantRecentJobsWidget extends WidgetBase
{
    ...
    public function frontend_render()
    {
        ...
        $blog_posts = Job::where(['status' => 1])->take($blog_items)->orderBy('id','desc')->get();

        $blogs_markup = '';
        foreach ($blog_posts as $post){

            $image = render_background_image_markup_by_attachment_id($post->image);
            $route = route('tenant.frontend.job.single',$post->slug);
